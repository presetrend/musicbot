        ...
        if name[0:3] == "sta":
            ...
            def get_stats():
                ...
                for vidid, count in list_arranged.items():
                    ...
                    if vidid == "telegram":
                        msg += f"🔗[File dan Audio Telegram](https://t.me/telegram) ** Dimainkan {count} waktu**\n\n"
                    else:
...
⚡️ __Music Dicari Oleh {config.MUSIC_BOT_NAME}__"""
...
async def welcome(client, message: Message):
    ...
    if config.PRIVATE_BOT_MODE == str(True):
        if not await is_served_private_chat(message.chat.id):
            await message.reply_text(
                "**Bot Musik Pribadi**\n\nHanya untuk obrolan resmi dari pemilik. Mintalah pemilik saya untuk mengizinkan obrolan Anda terlebih dahulu."
            )
